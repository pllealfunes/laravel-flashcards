describe("User login and create deck", () => {
    ...
    it("should show validation errors with invalid data", () => {
        ...
        ); // Adjust to match actual validation messages
    ...
    it("should create a deck successfully", () => {
        // Enter valid data
        cy.get('[data-testid="title-input-create"]').type("Test Deck");
        cy.get("button").contains("Add Flashcards").click();
        ...
        cy.contains("Successfully Created Deck."); // Adjust to match actual success message
        cy.url().should("include", "/dashboard"); // Adjust URL if needed
    });
});
